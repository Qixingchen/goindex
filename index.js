...
class googleDrive {
    ...
    async _ls(parent){
      ...
      while (obj && obj.nextPageToken && obj.nextPageToken.length > 0 && loadTimes < 30){
        ...
        obj.files = obj.files.concat(newObj.files);
        obj.nextPageToken = newObj.nextPageToken;
        loadTimes++;
      }

      console.log('_ls:' + JSON.stringify(obj));
      return obj;
    }

    async _lsWithToken(parent, pageToken){
      console.log("_ls_with_pageToken", parent + "  " + pageToken);
      ...
      let response = await fetch(url, requestOption);
      let obj = await response.json();
      // console.log('_ls_with_pageToken:' + JSON.stringify(obj));
      return obj;
    }
